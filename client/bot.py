...
class Bot:
    ...
    def check_message_intresting(self, message: custom.Message) -> bool:
        ...
        if media is None:
            ...
            return False
        if not isinstance(
            media, (types.MessageMediaPhoto, types.MessageMediaDocument)
        ):
            self.logger.info(
                'Skip message: media type "%s" is not intresting', type(media)
            )
            return False
    ...
    def apply_text_filters(self, message: custom.Message) -> None:
        if message.text:
            newtext = "\n".join(
                filter(
                    lambda s: not any(s.startswith(x) for x in self.filters),
    ...
    async def onNewMessage(self, event: events.NewMessage.Event) -> None:
        ...
        if msg.grouped_id:
            self.logger.info(
                "This is a group of media, i.e. Album, "
    ...
    def register_handlers(self):
        ...
        self.client.add_event_handler(
            self.onAlbum, events.Album(chats=self.channels)
        )
    ...
    async def _cache_and_get_entt(
        self, requested_entt: str
    ) -> Union["hints.Entity", None]:
        try:
    ...
    async def start(self, main_channel: str, sub_channel: str) -> None:
        ...
        if not isinstance(entt, types.Channel):
            raise ConfigurationException(
                "Provided name for main channel is not a channel, but: %s",
                type(entt),
            )
        ...
        if sub_channel:
            ...
            if entt is None:
                self.logger.error(
                    "Sub channel is given, but can't be found,"
                    "please check spelling"
                )
                ok = False
            if not isinstance(entt, types.Channel):
                self.logger.error(
                    "Provided name for sub channel is not a channel, but: %s",
                    type(entt),
                )
                ok = False
            if ok:
    ...
    async def get_meme_folder_id(self) -> int:
        ...
        try:
            meme_folder_id: int = next(
                ...
                filter(
                    lambda x: (
                        isinstance(x, types.DialogFilter)
    ...
    async def _main(self) -> None:
        ...
        await self._subscribe_channels(
            self.channels, subscribed, meme_folder_id
        )
    ...
    async def _clean_counter(self) -> None:
        ...
        tomorrow_begin = (today + timedelta(days=1)).replace(
    ...
    async def _mark_messages_readed(
        self, messages: list[custom.Message]
    ) -> None:
        awaitables = []
        for msg in messages:
    ...
    async def _check_limits_and_post_messages(
        self, messages: list[custom.Message], is_album=False
    ) -> None:
    ...
    async def _post_messages(
        ...
        try:
            if is_album:
                ...
                # to be iterable, but telethon is not bothering setting
                # right types
                await self.client._send_album(
                    channel,
                    messages,
            ...
            Exception
        ) as err:  # something wrong, but I don't want to die here
            self.logger.error(
                "Unexpected exception during message posting: %s", err
            )
        return False
