...
To bootstrap developer environment run `./bootstrap-dev.sh` from the project root.
This script will set virtual python environment, install required packages and set git hooks
...
To start app you need to run `bootstrap-run.sh` script with info from previous steps:
```
